...
**落在对话框里的下拉，层级必须排在对话框本体之上**（`Z_INDEX.DROPDOWN` > `Z_INDEX.MODAL_CONTENT`）。
Base UI 的下拉是 portal 到 body 上的，与对话框本体不是同一棵子树；排在下面就会被遮罩盖住，
症状是"能点开、能看见选项、就是点不中"。上游靠调用方自己传 `DROPDOWN_IN_MODAL` 解决，
本项目直接把这层定死在上面——对话框开着的时候主窗口里的下拉本来就打不开，不需要两档。
